...
function CameraMovementOverlay:move_camera(dt)
    local MOVEMENT_SPEED = 44

    local movement = util.input:read_wasd():normalize():mul(-MOVEMENT_SPEED)

    local limit_x = 2*5000 * self.camera_scale - love.graphics.getWidth() / 2 + 300
    local limit_y = 2*3000 * self.camera_scale - love.graphics.getHeight() / 2 + 200
