...
function CampaignGameEndOverlay:new(wrapped, timer_overlay, base_scene, pause_overlay, quit, next_level, star_times, register_time)
...
function CampaignGameEndOverlay:on_game_end()
    self.register_time(self.timer_overlay:get_time())

    GameEndOverlay.on_game_end(self)
end
...
function CampaignGameEndOverlay:get_gui()
    return gui({
        ...
        next = self.next
    }, self.timer_overlay:get_time(), self.star_times)
end
